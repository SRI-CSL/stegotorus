         ...
         compression_format fmt)
{
  ...
  if (slen > ZLIB_CEILING || dlen > ZLIB_CEILING) {
    log_warn("compression failure: slen = %" PRIuPTR " / dlen = %" PRIuPTR " > ZLIB_CEILING (%" PRIuPTR ")",
             slen, dlen, ZLIB_CEILING);
    return -1; 
  }
...
{
  if (slen > ZLIB_CEILING || dlen > ZLIB_CEILING) {
    log_warn("decompression failure: slen = %" PRIuPTR " / dlen = %" PRIuPTR " > ZLIB_CEILING (%" PRIuPTR ")",
             slen, dlen, ZLIB_CEILING);
    return -1;
  }

  if (slen > dlen) {
    log_warn("Attempting decompress of %" PRIuPTR " into buffer of %" PRIuPTR, slen, dlen);
  }
  ...
  if (ret != Z_STREAM_END) {
    log_debug("decompression failure: %s", strm.msg);
    inflateEnd(&strm);
    return -1;
  }
