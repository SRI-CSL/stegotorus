...
static bool _managed_mode = false;
static string _managed_method = "stegotorus";
...
void set_managed_mode(bool val) {
  _managed_mode = val;
}

void set_managed_method(string val) {
  _managed_method = val;
}
...
listener_open(struct event_base *base, config_t *cfg)
{
  ...
  log_debug("listener_open()");

  /* Open listeners for every address in the configuration. */
  for (i = 0; ; i++) {
    log_debug("listener_open[%" PRIuPTR "]", i);
    ...
    do {
      ...
      lsn->index = i;
      log_debug("attempting to listen on %s", lsn->address);
      lsn->listener =
      ...
      if (getsockname(evconnlistener_get_fd(lsn->listener), &tmpaddr.sockaddr, &len) != 0) {
	log_warn("couldn't get the bind addr");
	return 0;
      }
      
      listeners.push_back(lsn);

      log_warn("now listening on %s:%d for protocol %s", inet_ntoa(tmpaddr.in.sin_addr),
	       ntohs(tmpaddr.in.sin_port), cfg->name());

      if (_managed_mode) {
	fprintf(stdout, "VERSION 1\n");
	fprintf(stdout, "CMETHOD %s socks4 %s:%d\n", _managed_method.c_str(), inet_ntoa(tmpaddr.in.sin_addr),
		ntohs(tmpaddr.in.sin_port));
	fprintf(stdout, "CMETHODS DONE\n");

	log_warn("VERSION 1\n");
        log_warn("CMETHOD %s socks4 %s:%d\n", _managed_method.c_str(),
                 inet_ntoa(tmpaddr.in.sin_addr),
                 ntohs(tmpaddr.in.sin_port));
  	log_warn("CMETHODS DONE\n");
...
downstream_event_cb(struct bufferevent *bev, short what, void *arg)
{
  ...
  if (what & (BEV_EVENT_ERROR|BEV_EVENT_EOF|BEV_EVENT_TIMEOUT)) {
    ...
    if (what == (BEV_EVENT_EOF|BEV_EVENT_READING)) {
      ...
      if (ckt == NULL || ckt->cfg()->mode == LSN_SOCKS_CLIENT || (conn->read_eof && conn->write_eof))
        conn->close();
    } else {
      conn->close();
    }
