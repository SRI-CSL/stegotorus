...
write_msg_chains(flow *f, FILE *file, pentry_header *ph)
{
  ...
  if (strnstr((char*) m->buf, "200 OK",  m->len) &&
      strnstr((char*) m->buf, "Content-Encoding: gzip",  m->len))
...
main(int argc, char **argv)
{
  ...
  client_file = fopen(STEG_TRACES_DIR "client.out", "w");
  if (!client_file) {
    perror(STEG_TRACES_DIR "client.out");
    return 1;
  }
  server_file = fopen(STEG_TRACES_DIR "server.out", "w");
  if (!server_file) {
    perror(STEG_TRACES_DIR "server.out");
    return 1;
  }
