...
namespace {
  ...
  {
    EVP_CIPHER_CTX ctx;
    ecb_encryptor_impl() : ctx() { EVP_CIPHER_CTX_init(&ctx); }
  ...
  {
    EVP_CIPHER_CTX ctx;
    ecb_decryptor_impl() : ctx()  { EVP_CIPHER_CTX_init(&ctx); }
...
namespace {
  ...
  {
    EVP_CIPHER_CTX ctx;
    gcm_encryptor_impl()  : ctx() { EVP_CIPHER_CTX_init(&ctx); }
  ...
  {
    EVP_CIPHER_CTX ctx;
    gcm_decryptor_impl()  : ctx() { EVP_CIPHER_CTX_init(&ctx); }
...
namespace {
  struct ecdh_message_impl : ecdh_message
  {
    ...
    DISALLOW_COPY_AND_ASSIGN(ecdh_message_impl);
