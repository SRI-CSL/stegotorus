...
public:
  ...
  DISALLOW_COPY_AND_ASSIGN(conn_global_state);
            ...
            struct bufferevent *buf, const char *peername)
{
  ...
  /* Fail when we are in the process of shutting down */
  if (cgs->shutting_down) {
      return NULL;
  }
...
circuit_t::close()
{
  ...
  if (this->flush_timer) {
    assert(event_initialized(this->flush_timer));
    event_del(this->flush_timer);
  }

  if (this->axe_timer) {
    assert(event_initialized(this->axe_timer));
...
circuit_recv_eof(circuit_t *ckt)
{
  ...
  if (outlen) {
    log_warn(ckt, "flushing %lu bytes to upstream", (unsigned long)outlen);
    circuit_do_flush(ckt);
    return;
  }
