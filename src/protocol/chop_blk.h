...
const size_t ENC_HANDSHAKE_LEN = 32; // size of encrypted handshake
...
public:
  ...
  void set_block_received(uint32_t seq)
  {
    ...
    uint32_t delta;
    if (seq < hsn_)
       delta = seq + 256 - hsn_ - 1;
    else
       delta = (seq - hsn_) - 1;
    if (delta >= 256)
      return;
    //      log_abort("seq %u too high (hsn %u) delta %u", seq, hsn_, delta);
...
struct transmit_elt
 ...
 transmit_elt() : hdr(), data(0), last_sent() { }
 };
 ...
 class transmit_queue
 {
   ...
   DISALLOW_COPY_AND_ASSIGN(transmit_queue);
 ...
 public:
    ...
    */
   bool full() const
   { if (next_to_send - next_to_ack > 255)
       log_warn("queue full nts: %u nta %u diff %u \n", next_to_send, next_to_ack, next_to_send - next_to_ack);
     return next_to_send - next_to_ack > 255;
    }


   bool busy() const
   {      return next_to_send - next_to_ack > 128;
    }
   ...
   public:
     ...
     transmit_elt& peek(uint32_t s)  { return queue->cbuf[s & 0xFF]; }
...
public:
  ...
  uint32_t length() const { return count; }
