...
static bool managed_mode = false;

static string pidfile_name;
static string registration_helper;

static const char project_git_orig[] = "GIT origin: " PROJECT_GIT_ORIG;
static const char project_git_brch[] = "GIT branch: " PROJECT_GIT_BRCH;
static const char project_git_hash[] = "GIT hash: " PROJECT_GIT_HASH;
static const char project_git_time[] = "GIT time: " PROJECT_GIT_TIME;
...
{
/* XXX: We only want ST as a client on Windows, thus not fixing this yet */
#ifndef _WIN32
...
#else
  log_warn("Windows does not support subprocesses (%s)", helper.c_str());
#endif
}
...
usage(void)
{
  ...
  fprintf(stderr, "\n* Available arguments:\n"
          ...
          "--managed ~ run as a managed pluggable transport\n"
...
handle_generic_args(const char *const *argv,  modus_operandi_t &mo)
{
  ...
  while (argv[i] && !strncmp(argv[i],"--",2)) {
    if (!strncmp(argv[i], "--config-file=", strlen("--config-file="))) {
      ...
      mo.load_file(path);
      if (!mo.is_ok()){
        fprintf(stderr, "The configation file  \"%s\" did not load smoothly!\n", path);
        exit(1);
      }
      ...
      daemon_mode = true;
    } else if (!strcmp(argv[i], "--managed")) {
      if (managed_mode) {
        fprintf(stderr, "you've already requested managed mode!\n");
        exit(1);
      }
      managed_mode = true;
      mo.managed(true);
  ...
  */
  ...
  if(mo.is_ok()){
    ...
    set_managed_mode(mo.managed());
    set_managed_method(mo.managed_method());
