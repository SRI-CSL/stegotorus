...
namespace {
  ...
  struct embed_steg_t : steg_t {
    ...
    DISALLOW_COPY_AND_ASSIGN(embed_steg_t);
...
embed_steg_config_t::embed_steg_config_t(config_t *cfg)
  : steg_config_t(cfg), is_clientside(cfg->mode != LSN_SIMPLE_SERVER), traces()
{
  // read in traces to use for connections
  FILE *trace_file = fopen(STEG_TRACES_DIR "embed.txt", "r");
  if (!trace_file)
...
void
embed_steg_t::successful_reception() {};

unsigned int
embed_steg_t::corrupted_reception() {return 0;};
...
embed_steg_t::embed_steg_t(embed_steg_config_t *cf, conn_t *cn)
  : config(cf), conn(cn), cur_idx(0), cur(NULL), cur_pkt(0), last_pkt()
{
  cur_idx = -1;
...
embed_steg_t::transmit(struct evbuffer *source)
{
  ...
  if (cur_pkt == 0) {
    if (evbuffer_add(dest, &cur_idx, 4) == -1) return NOT_TRANSMITTED;
    used += 4;
  ...
  if (evbuffer_add(dest, &src_len, 2) == -1) return NOT_TRANSMITTED;
  if (evbuffer_add_buffer(dest, source) == -1) return NOT_TRANSMITTED;
...
embed_steg_t::receive(struct evbuffer *dest)
{
  ...
  if (cur_idx == -1) {
    if (evbuffer_remove(source, &cur_idx, 4) != 4) return RECV_BAD;
    cur = &config->traces[cur_idx];
    cur_pkt = 0;
    pkt_size += 4;
  ...
  while (1) {
    ...
    // read data
    short data_len;
    if (evbuffer_remove(source, &data_len, 2) != 2) return RECV_BAD;
    if (data_len > 0) {
    ...
    if (exp_pkt_size > pkt_size) {
      ...
      if (evbuffer_drain(source, padding) == -1) return RECV_BAD;
    }
