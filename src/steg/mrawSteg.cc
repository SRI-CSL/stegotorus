...
static
size_t construct_mraw_headers(stream_steg_t *s);

static 
size_t construct_mraw_part_headers(stream_steg_t *s, size_t body_length, char* pheaders);

transmit_t
stream_server_MRAW_transmit (stream_steg_t *s, struct evbuffer *dest, struct evbuffer *source){
  ...
  if((s->headers_out == NULL) && !s->headers_sent){
    ...
    s->headers_out_length = construct_mraw_headers(s);

    if(s->headers_out_length == 0){
      log_warn("construct_mraw_headers failed.");
      goto clean_up;
    }
  ...
  pheaders =  (char *)xzalloc(MRAW_MAX_PART_HEADERS_SIZE);
  pheaders_length = construct_mraw_part_headers(s, data_length, pheaders);
  ...
  if(SCHEMES_PROFILING){
    profile_data("RAW", pheaders_length, data_length, source_length);
  }
...
recv_t
stream_client_MRAW_receive (stream_steg_t * s, struct evbuffer *dest, struct evbuffer *source)
{
...
size_t
construct_mraw_part_headers(stream_steg_t * s, size_t body_length, char* pheaders){
  log_debug("%p %" PriSize_t " %p",  s, body_length, pheaders);

  snprintf(pheaders, MRAW_MAX_PART_HEADERS_SIZE, "--myboundary %" PriSize_t  " %" PriSize_t  "\r\nContent-Type: image/jpeg\r\nContent-Length: %d\r\n\r\n", packet_id++, s->stream_id, (int)body_length);

  return strlen(pheaders);
}

size_t
construct_mraw_headers(stream_steg_t *s){
...
transmit_t
stream_client_MRAW_transmit (stream_steg_t * s, struct evbuffer *dest, struct evbuffer *source){
  transmit_t retval = NOT_TRANSMITTED;
  size_t source_length = evbuffer_get_length(source);
  unsigned char *data = NULL;
  char *outbuf = NULL;
  ...
  if (source2raw(source, source_length, &data, datalen) != RCODE_OK) {
    log_warn("source2raw called returned negative value");
    goto clean_up;
  }
  ...
  */
  ...
  // loop till success
  while (stream_gen_uri(outbuf, outbufsz, (char *)data, datalen) != RCODE_OK){ };
...
recv_t
stream_server_MRAW_receive (stream_steg_t * s, struct evbuffer *dest, struct evbuffer *source){
  /* here to stop whining about unused variables sheesh! */
  log_debug("%p %p %p", s, dest, source);
  recv_t retval = RECV_BAD;
